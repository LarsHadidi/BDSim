...
/** Returns a vector of the ordered vertex vectors in the global coordinate system.
 * \param[out] v the vector to store the results in.
 * \param[in] (x,y,z) the position vector of the particle in the global
 *                    coordinate system. */
void voronoicell_base_2d::vertices_in_order(double x,double y,vector<double> &v) {
	v.resize(2*p);
	if(p==0) return;
	int k=0;
	int i=0;
